<template>
    ...
    <div class="content-header">
        ...
        <span class="switch-mode">
            <span @click="switchCard = 0" title="List" :class="switchCard === 0 ? 'listcard-act' : ''"><font-awesome-icon
                    icon="fa-solid fa-list" /></span>
            <span @click="switchCard = 1" title="Card" :class="switchCard === 1 ? 'listcard-act' : ''"><font-awesome-icon
                    icon="fa-solid fa-border-all" /></span>
            |
        ...
        <table>
            ...
            <tbody>
                ...
                <tr v-for="(data, index) in listdata.value" :key="data.id">
                    <td>{{ index + 1 }}</td>
                    <td><img src="../../../assets/image/IMG_20211214_194210-picsay.jpg" alt="Pict" @click="modalImg = true">
                    </td>
                     <td>{{ data.nis }}</td>
                        <td>{{ data.name }}</td>
                        <td>{{ data.email }}</td>
        ...
        <div class="table-card">
                <div class="card-data" v-for="(data, index) in listdata.value" :key="data.id">
                    ...
                    <div class="text-content">
                        <p>{{ data.name }}</p>
                        <p>{{ data.nis }}</p>
                        <p>{{ data.major.name }} | {{ data.region.name }}</p>
                    </div>
...
export default {
    ...
    setup() {
        ...
        const refresh = async () => {
            const res = await axios.get(apiHost + 'students?=STUDENT')
